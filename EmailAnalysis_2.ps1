    ...
    if($intCount -gt 0) {
        ...
        for($i=1; $i -le $intCount; $i++) {
            ...
            if($attachment.FileName -like "*.zip"){
                ...
                expand-archive -path $attachmentPath -outputpath $filepath}
                }
                }
                }
...
python "C:\Users\l_li\Desktop\Python Project\ExcelAnalysis\ExcelAnalysis\ExcelCSQ.py"      ############
sleep 2
python "C:\Users\l_li\Desktop\Python Project\ExcelAnalysis\ExcelAnalysis\ExcelPeformance.py"
sleep 2
...
{
    if ($item.name -match "^Oxford*" -or $item.name -match "^Daily*")
    {
